...
## Summary
Incidence and severity were forecast for *Zymoseptoria tritici* and yellow rust on wheat leaves 1 and 2 across nine UK regions, from a per region-year feature table of weather, sowing date, fungicide use and spatial adjacency. Four learning algorithms (XGBoost, LightGBM, CatBoost, penalised GLM) were compared on identical data, then six forecast methods — naive baselines, machine learning and blends tested per target. Each target forecast by whichever method won its rolling-origin backtest, XGBoost for Septoria, EWMA smoothing for yellow rust.

## Season window
The season window runs 1st October to 20th June. 1st October approximates the start of sowing, 20th June is a proxy for the end of assessment.

## Weather data
Daily temperature, precipitation, relative humidity and wind speed were pulled per region from the Open-Meteo historical API from 1970, then aggregated into seasonal windows.
...
## Modelling
**Learning algorithm.** The four algorithms named above were compared on the same feature table, the three tree methods each over five random seeds so differences could be read against seed noise. Reproducible via `analysis/evidence/model_comparison.py`, results in `model_comparison_results.csv`.

**Forecast method.** Six ways of turning the winning algorithm into a forecast were compared: persistence, a 5-year rolling mean, an EWMA smoother, XGBoost on mechanistic features only, XGBoost on all features, and a mechanistic/rolling-mean blend.
...
The intervals are wide — 90% bounds on Septoria incidence, leaf 1 run 34-99% — and that width is the substantive result: the year effect carries most of the signal and is near-unpredictable, so a one-year-ahead forecast speaks to central tendency, not next season's outcome. Per-region intervals are in `submission/pest_forecasts_2026_intervals.csv`, coverage detail in `analysis/evidence/uncertainty_coverage.csv`, both from `analysis/evidence/uncertainty.py`.
